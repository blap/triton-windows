# run_benchmarks_sm61.ps1 - PyTorch vs Triton Performance Benchmarks on SM61

# Ensure sm61 directory exists
if (!(Test-Path "sm61")) {
    New-Item -ItemType Directory -Path "sm61" | Out-Null
}

Write-Host "Starting PyTorch vs Triton Performance Benchmarks on SM61..." -ForegroundColor Cyan
...
# Save benchmark script to file
$benchmarkScript | Out-File -FilePath "sm61\benchmark_script.py" -Encoding UTF8

# Run benchmark and save results
Write-Host "Running benchmarks..." -ForegroundColor Yellow
$benchmarkResult = python "sm61\benchmark_script.py" 2>&1

# Save benchmark results
$benchmarkResult | Out-File -FilePath "sm61\benchmark_results.txt"

# Display results
Write-Host "\nBenchmark Results:" -ForegroundColor Cyan
Write-Host "==================" -ForegroundColor Cyan
$benchmarkResult

Write-Host "\nBenchmark results saved to sm61\benchmark_results.txt" -ForegroundColor Green
Write-Host "Benchmark script saved to sm61\benchmark_script.py" -ForegroundColor Green
Write-Host "\nBenchmarks completed successfully!" -ForegroundColor Green