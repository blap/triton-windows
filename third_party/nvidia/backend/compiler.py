...
class CUDABackend(BaseBackend):
    ...
    def make_llir(self, src, metadata, options, capability):
        ...
        # For sm61 (Pascal), skip MMA-specific passes that are not supported
        if capability >= 70:  # SM 70+
        ...
        # For sm61, skip tensor memory allocation as it's not supported
        if capability >= 70:  # SM 70+
            nvidia.passes.ttnvgpuir.add_allocate_tensor_memory(pm)
        passes.ttgpuir.add_allocate_global_scratch_memory(pm)
        # Use the correct function name for the LLVM IR conversion
        if hasattr(nvidia.passes.ttnvgpuir, 'add_nvgpu_to_llvm'):
            nvidia.passes.ttnvgpuir.add_nvgpu_to_llvm(pm)
        passes.common.add_canonicalizer(pm)
        passes.common.add_cse(pm)
        # For sm61, skip NVGPU-specific passes that are not supported
        if capability >= 70:  # SM 70+
            if hasattr(nvidia.passes.ttnvgpuir, 'add_warp_specialize_to_llvm'):
